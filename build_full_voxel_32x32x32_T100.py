...
OUT_DIR = Path(r"C:\Users\HP\Searches\fmri_datasets\full_voxel\32x32x32_T100")
...
def resize_3d_volume(volume_3d: np.ndarray, target_shape: tuple[int, int, int]) -> np.ndarray:
    """Resize 3D wolumenu do zadanego kształtu."""
...
def standardize_subject(data_4d: np.ndarray) -> np.ndarray:
    """Prosta standaryzacja per subject."""
    mean = data_4d.mean()
    std = data_4d.std()
    if std < 1e-8:
...
for f in all_files:
    ...
    # mapowanie: ASD=1 -> 1, Control=2 -> 0
    y = 1 if dx == 1 else 0

    eligible_files.append(f)
    eligible_labels.append(y)

print("Liczba subjectów z T >= 100:", len(eligible_files))
